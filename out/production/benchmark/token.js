...
module.exports = function(requestId) {
    ...
    const zone = Math.random() < 0.9 ? "1" : "7"
    const vz = Math.random() < 0.9 ? "123" : "7"
    const exp = Math.random() < 0.9 ? now+1 : 1516239022
    const privateKey = Math.random() < 0.9 ? correctPrivateKey : incorrectPrivateKey
    const sub = Math.random() < 0.5 ? String(i++).padStart(10, '0') : "0000000001"
